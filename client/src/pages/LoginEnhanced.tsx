...
import { ArrowLeft, CheckCircle2, Eye, EyeOff, Github, Loader2, LockKeyhole, Mail, Phone, ShieldCheck, Sparkles } from "lucide-react";
    ...
    <section className="relative z-10 w-full max-w-sm">
      ...
      <div className="rounded-2xl border border-white/10 bg-[#141417]/90 p-6 shadow-2xl backdrop-blur">
        ...
        {registrationPending ? <div className="py-8 text-center"><CheckCircle2 className="mx-auto h-10 w-10 text-[#10b981]" /><h2 className="mt-4 text-xl font-semibold">Confirm your email</h2><p className="mt-2 text-sm leading-6 text-[#9a9aa2]">We sent a 30-minute verification link to <span className="text-[#f5f5f7]">{email}</span>. Verify it before using email/password sign-in.</p><Button variant="outline" className="mt-5 border-white/10" disabled={resend.isPending} onClick={() => resend.mutate({ email })}>Resend verification</Button></div> : <form className="mt-6 space-y-4" onSubmit={submit}>
          ...
          {mode === "signin" && <div className="space-y-2"><label className="flex cursor-pointer items-start gap-2 text-sm text-[#d4d4d8]"><input checked={rememberMe} onChange={(event) => setRememberMe(event.target.checked)} type="checkbox" className="mt-0.5 h-4 w-4 rounded border-white/20 bg-[#09090b] accent-[#8b5cf6]" /><span>Remember me for 30 days<span className="mt-0.5 block text-xs text-[#9a9aa2]">Leave unchecked on shared devices. Your session ends when the browser closes or after 12 hours.</span></span></label><button type="button" onClick={() => navigate("/forgot-password")} className="text-sm text-[#a78bfa] transition-colors hover:text-white">Forgot password?</button></div>}
        ...
        <Button variant="outline" type="button" disabled className="mt-3 w-full border-white/10 text-[#9a9aa2]"><Github className="mr-2 h-4 w-4" />GitHub sign-in coming soon</Button>
