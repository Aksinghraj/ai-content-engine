...
export default function BlogPost() {
  ...
  <section className="mt-12 rounded-2xl border border-[#26262b] bg-[#141417] p-6"><div className="flex flex-col gap-5 sm:flex-row sm:items-center"><img src={founder.image} alt={founder.name} width={96} height={96} className="h-20 w-20 rounded-full object-cover ring-1 ring-[#26262b]" /><div><p className="font-semibold text-[#f5f5f7]">{founder.name}</p><p className="text-sm text-[#06b6d4]">{founder.title}</p><p className="mt-2 text-sm leading-relaxed text-[#9a9aa2]">{founder.bio}</p><a href={founder.instagram} target="_blank" rel="noreferrer" className="mt-3 inline-flex items-center gap-1.5 text-sm text-[#c4b5fd] hover:text-white"><Instagram className="h-4 w-4" />Follow Veer on Instagram</a></div></div></section>
