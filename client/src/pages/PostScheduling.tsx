...
export default function PostScheduling() {
  ...
  const [scheduleResults, setScheduleResults] = useState<{ platform: string; success: boolean; detail: string }[]>([]);
  ...
  const schedulePost = async () => {
    ...
    const results = await Promise.allSettled(platforms.map(async (platform) => {
      ...
      await createSchedule.mutateAsync({ socialConnectionId: connection.id, platform, content, scheduledAt: time, mediaUrl: media?.url, mediaType: media?.mediaType, mediaKey: media?.key });
      return platform;
    }));
  ...
  return <DashboardLayout><main className="mx-auto w-full max-w-7xl space-y-5 px-4 py-5 sm:px-6 lg:px-8">
    ...
    <Tabs value={tab} onValueChange={setTab} className="space-y-5"><TabsList className="grid h-auto w-full grid-cols-3 bg-muted p-1"><TabsTrigger value="compose" className="min-h-10 px-2 text-xs sm:text-sm">Compose</TabsTrigger><TabsTrigger value="generate" className="min-h-10 px-2 text-xs sm:text-sm">AI generate</TabsTrigger><TabsTrigger value="scheduled" className="min-h-10 px-2 text-xs sm:text-sm">Scheduled</TabsTrigger></TabsList>
      ...
      {scheduleResults.length > 0 && <Card className="border-border bg-card"><CardHeader className="pb-2"><CardTitle className="text-base">Schedule result</CardTitle><CardDescription>Each result below is from the scheduling API.</CardDescription></CardHeader><CardContent className="space-y-2">{scheduleResults.map((result) => <div key={result.platform} className="flex items-start gap-2 rounded-md border border-border bg-background p-3 text-sm"><span className="mt-0.5">{result.success ? <CheckCircle2 className="h-4 w-4 text-emerald-500" /> : <AlertCircle className="h-4 w-4 text-rose-500" />}</span><span><strong>{result.platform}</strong><span className="ml-2 text-muted-foreground">{result.detail}</span></span></div>)}</CardContent></Card>}
