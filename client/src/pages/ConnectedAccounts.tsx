...
import { AlertCircle, CheckCircle2, ExternalLink, Link2, Loader2, RefreshCw, ShieldCheck, Unlink2, XCircle } from "lucide-react";
...
export default function ConnectedAccounts() {
  ...
  return <DashboardLayout><main className="mx-auto w-full max-w-7xl space-y-5 px-4 py-5 sm:px-6 lg:px-8">
    ...
    {accountsQuery.isLoading ? <Card className="border-border bg-card p-10 text-center text-muted-foreground"><Loader2 className="mx-auto h-6 w-6 animate-spin" /><p className="mt-3 text-sm">Checking connected accounts</p></Card> : <section className="grid grid-cols-1 gap-4 md:grid-cols-2">{PLATFORMS.map((platform) => {
      ...
      const blockingGuidance = account?.canEnableAutoPost === false ? PROVIDER_GUIDANCE.twitter : PROVIDER_GUIDANCE[platform.id] || "Reconnect and validate this account before Auto-Post can be enabled.";
      return <Card key={platform.id} className="flex min-h-72 flex-col border-border bg-card text-card-foreground"><CardHeader className="pb-3"><div className="flex items-start justify-between gap-3"><div className="flex min-w-0 gap-3"><span className="text-2xl" aria-hidden="true">{platform.icon}</span><div><CardTitle className="text-base">{platform.name}</CardTitle><CardDescription className="mt-1 text-xs leading-5">{platform.description}</CardDescription></div></div><Badge variant="outline" className={statusClass}>{status.tone === "ready" ? <CheckCircle2 className="mr-1 h-3.5 w-3.5" /> : status.tone === "warning" ? <AlertCircle className="mr-1 h-3.5 w-3.5" /> : <XCircle className="mr-1 h-3.5 w-3.5" />}{status.label}</Badge></div></CardHeader><CardContent className="flex flex-1 flex-col gap-3"><div className="min-h-16 rounded-md border border-border bg-background/40 p-3 text-xs leading-5 text-muted-foreground">{account?.username && <p className="mb-1 text-sm font-medium text-foreground">@{account.username}</p>}<p>{status.detail}</p>{account?.tokenExpiresAt && status.tone !== "neutral" && <p className="mt-1">Token expiry: {new Date(account.tokenExpiresAt).toLocaleDateString()}</p>}</div>{account ? <><div className="flex items-center justify-between gap-3 rounded-md border border-border px-3 py-2"><div><p className="text-sm font-medium">Auto-Post</p><p className="text-xs text-muted-foreground">Required for scheduled publishing</p></div><Switch checked={Boolean(account.autoPost)} disabled={!canToggle || setAutoPost.isPending} onCheckedChange={(enabled) => toggleAutoPost(platform.id, enabled)} aria-label={`Toggle Auto-Post for ${platform.name}`} /></div>{!canToggle && <p className="text-xs text-amber-500">{blockingGuidance}</p>}
<div className="mt-auto grid grid-cols-2 gap-2"><Button type="button" variant="outline" size="sm" onClick={() => refreshAccount(platform.id)} disabled={busy || refresh.isPending}><RefreshCw className="mr-1.5 h-4 w-4" />Refresh</Button><Button type="button" variant="outline" size="sm" className="border-destructive/40 text-destructive hover:text-destructive" onClick={() => disconnectAccount(platform.id)} disabled={busy}>{disconnecting === platform.id ? <Loader2 className="mr-1.5 h-4 w-4 animate-spin" /> : <Unlink2 className="mr-1.5 h-4 w-4" />}Disconnect</Button></div></> : <><div className="min-h-9 text-xs text-amber-500">{PROVIDER_GUIDANCE[platform.id] || "Provider approval may be required before connection is available."}</div><Button type="button" className="mt-auto w-full bg-gradient-to-r from-indigo-500 via-violet-500 to-cyan-500 text-white hover:opacity-90" onClick={() => connect(platform.id)} disabled={busy}>{connecting === platform.id ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Link2 className="mr-2 h-4 w-4" />}{connecting === platform.id ? "Opening provider…" : "Connect"}</Button></>}</CardContent></Card>;
    })}</section>}
    <section className="grid gap-4 lg:grid-cols-2"><Card className="border-amber-500/30 bg-amber-500/5 text-card-foreground"><CardHeader className="pb-3"><CardTitle className="text-base">Meta activation checklist</CardTitle><CardDescription>These provider steps are required before customer Instagram or Facebook connections can become ready.</CardDescription></CardHeader><CardContent className="space-y-2 text-sm leading-6 text-muted-foreground"><p>Complete Business Verification, add the Instagram API Content Publishing use case, request the required advanced permissions, and switch the Meta app to Live mode after approval.</p><a className="inline-flex items-center gap-1.5 text-primary hover:underline" href="https://developers.facebook.com/apps/" target="_blank" rel="noreferrer">Open Meta Developers <ExternalLink className="h-3.5 w-3.5" /></a><p className="text-xs">After approval, reconnect Instagram here so Lumae receives a fresh validated token. Do not enable Auto-Post on a tester-only or unvalidated connection.</p></CardContent></Card><Card className="border-blue-500/30 bg-blue-500/5 text-card-foreground"><CardHeader className="pb-3"><CardTitle className="text-base">LinkedIn readiness checklist</CardTitle><CardDescription>LinkedIn text publishing uses the current REST posts API.</CardDescription></CardHeader><CardContent className="space-y-2 text-sm leading-6 text-muted-foreground"><p>Enable OpenID Connect and Share on LinkedIn in the LinkedIn Developer Portal, register the production callback, then connect the profile again.</p><a className="inline-flex items-center gap-1.5 text-primary hover:underline" href="https://www.linkedin.com/developers/apps" target="_blank" rel="noreferrer">Open LinkedIn Developers <ExternalLink className="h-3.5 w-3.5" /></a><p className="text-xs">Lumae currently allows text-only LinkedIn publishing; media uploads remain intentionally blocked until supported end to end.</p></CardContent></Card></section><Card className="border-primary/30 bg-primary/5 text-card-foreground"><CardContent className="flex gap-3 p-4"><ShieldCheck className="mt-0.5 h-5 w-5 shrink-0 text-primary" /><div className="text-sm leading-6"><p className="font-medium">What Lumae checks</p><p className="text-muted-foreground">Account identity and permissions are validated server-side. Tokens stay encrypted, are never displayed in this page, and are removed when you disconnect. A provider or app-review restriction may still prevent a connection from becoming ready.</p></div></CardContent></Card>
