...
export function PlatformAutomationWorkspace() {
  ...
  const { data: engagementEvents = [], isLoading: eventsLoading } = trpc.enterprise.getEngagementEvents.useQuery({ limit: 100 });
  ...
  const selectedEvents = useMemo(() => engagementEvents.filter((event: any) => event.platform === activeId), [engagementEvents, activeId]);
  ...
  return <main className="container space-y-8 py-8 sm:py-10">
    ...
    <div className="grid gap-5 xl:grid-cols-3">
      ...
      <Card><CardHeader><CardTitle className="flex items-center gap-2 text-lg"><MessageCircleReply className="h-5 w-5 text-primary" />Comment reply review</CardTitle><CardDescription>{workspace.replyDetail}</CardDescription></CardHeader><CardContent className="space-y-4"><StatusBadge kind={workspace.replyAvailable ? "warning" : "locked"} label={workspace.replyAvailable ? "Manual review only" : "Execution locked"} /><p className="text-sm text-muted-foreground">No provider comment event is fetched or acted on until the relevant connection and approved event workflow are enabled.</p><Button variant="outline" className="w-full" onClick={() => navigate("/automation/reply-inbox")}><Eye className="mr-2 h-4 w-4" />{selectedEvents.length ? "Open review queue" : "Open Reply Inbox"}</Button></CardContent></Card>
      <Card><CardHeader><CardTitle className="flex items-center gap-2 text-lg"><Sparkles className="h-5 w-5 text-primary" />Workspace analytics</CardTitle><CardDescription>Only operational data that Lumae can verify is shown.</CardDescription></CardHeader><CardContent className="grid grid-cols-2 gap-3"><Metric label="Schedules" value={selectedSchedules.length} /><Metric label="Active" value={activeCount} /><Metric label="Connection" value={status.kind === "ready" ? "Ready" : "Locked"} /><Metric label="Reply events" value={eventsLoading ? "…" : selectedEvents.length} /></CardContent></Card>
    </div>
