...
export function TwoFactorSecurityPanel() {
  ...
  const revokeTrustedDevice = trpc.twoFactor.revokeTrustedDevice.useMutation({
    onSuccess: async () => {
      await utils.twoFactor.status.invalidate();
  ...
  const revokeAllTrustedDevices = trpc.twoFactor.revokeAllTrustedDevices.useMutation({
    onSuccess: async () => {
      await utils.twoFactor.status.invalidate();
  ...
  if (status?.enabled) {
    return (
      <Card className="border-border bg-card p-6">
        ...
        <div className="mt-4 rounded-xl border border-border bg-muted/25 p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-medium text-card-foreground">Trusted devices</p><p className="mt-1 text-xs text-muted-foreground">Trusted devices can skip the second-factor prompt until their chosen expiry. They do not use emails or temporary-login links.</p></div><ShieldCheck className="h-5 w-5 shrink-0 text-primary" /></div>{status.trustedDevices.length === 0 ? <p className="mt-3 text-xs text-muted-foreground">No trusted devices are active.</p> : <div className="mt-3 space-y-2">{status.trustedDevices.map((device) => <div key={device.id} className="flex flex-col justify-between gap-2 rounded-lg border border-border bg-background p-3 sm:flex-row sm:items-center"><p className="text-xs text-muted-foreground">Expires {new Date(device.expiresAt).toLocaleString()} {device.lastUsedAt ? `· Last used ${new Date(device.lastUsedAt).toLocaleString()}` : ""}</p><Button size="sm" variant="outline" className="border-border" disabled={revokeTrustedDevice.isPending} onClick={() => revokeTrustedDevice.mutate({ deviceId: device.id })}>Revoke</Button></div>)}</div>} {status.trustedDevices.length > 1 && <Button size="sm" variant="outline" className="mt-3 border-[#ef4444]/45 text-[#ef4444] hover:bg-[#ef4444]/10" disabled={revokeAllTrustedDevices.isPending} onClick={() => revokeAllTrustedDevices.mutate()}>Revoke all devices</Button>}</div>
