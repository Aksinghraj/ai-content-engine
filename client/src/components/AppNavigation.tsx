  ...
  return <section className={`lumae-group-tabs ${isContentStudio ? "lumae-group-tabs--complete" : ""}`} aria-label={`${activeArea.label} sections`}>
    ...
    <div className={`lumae-group-tabs__rail ${isContentStudio ? "lumae-group-tabs__rail--complete" : ""}`} role="tablist" aria-label={`${activeArea.label} tabs`}>
      {activeArea.tabs.map((tab, index) => {
        ...
        return <div className={`lumae-tool-card ${isFavorite ? "is-favorite" : ""}`} key={tab.path}><Link href={tab.path} onClick={() => recordRecent(tab.path)} role="tab" aria-selected={active} className={`lumae-group-tab ${active ? "is-active" : ""}`}><i>{String(index + 1).padStart(2, "0")}</i>{ToolIcon && <ToolIcon className="lumae-group-tab__icon h-4 w-4" aria-hidden="true"/>}<span>{tab.label}</span>{tool && <small>{tool.description}</small>}</Link>{isContentStudio && <button type="button" className="lumae-tool-pin" onClick={(event) => toggleFavorite(event, tab.path)} aria-label={`${isFavorite ? "Remove" : "Add"} ${tab.label} ${isFavorite ? "from" : "to"} favorites`} aria-pressed={isFavorite}><Star className="h-3.5 w-3.5" fill={isFavorite ? "currentColor" : "none"}/></button>}</div>;
      })}
    </div>
