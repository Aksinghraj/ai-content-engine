...
describe("language and OAuth recovery", () => {
  ...
  it("keeps Bhojpuri selectable, persisted, and explicitly requested by the server generator", () => {
    ...
    expect(generator).toContain("LANGUAGE_OPTIONS");
    expect(languageContext).toContain('code: "bho", name: "Bhojpuri"');
    ...
    expect(languageContext).toContain('code: "as", name: "Assamese"');
    expect(languageContext).toContain('code: "ur", name: "Urdu"');
    expect(preferenceRouter).toContain('"as"');
    expect(preferenceRouter).toContain('"ur"');
